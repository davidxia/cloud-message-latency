...
func main() {
	...
	go receive(sub, ctx, burnInStart, summary, errChan)
...
func receive(sub *pubsub.Subscription, ctx context.Context, burnInStart int64, summary prometheus.Summary, errChan chan error) {
	...
	err := sub.Receive(ctx, func(ctx context.Context, msg *pubsub.Message) {
		...
		//log.Printf("got message %s at %d, latency %.0f ms", dataStr, receiptTime, latencyMillis)
		msg.Ack()
	})
	if err != nil {
		log.Fatalf("error pulling messages from subscription %s", *subscriptionID)
	}
	errChan <- err
}
